...
- **Tag conventions (this repo).** `backlog` = not in the live queue — every deferred task carries it,
  and dropping it is what "promoting" means (see above); the live queue is exactly `list` minus
  `backlog`. `think` = design-open items ("Think:" titles) — brainstorm before implementing. Thematic
  tags are a deliberately SMALL vocabulary — currently `core`, `flow`, `sec`, `tests`, `perf`, `dx`,
  `ux`, `kb`, `adapter`, `demo`, `multiagent`, `release`, `docs` — pick from the existing set before
  inventing (discover the live set via `mtt list --json | jq -r '.[].tags[]?' | sort | uniq -c` until
  `mtt tags` (t39) lands). Caveat: `#hashtags` in titles/descriptions auto-become tags — never put `#`
  in a title unless you mean it. (Policy home migrates into mtt when t29 lands.)
