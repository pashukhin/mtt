...
| 009 | **dogfood** ⬅ **next** | self-host — **starts by reconciling the spec with decision A** (full session flow `tbd → speccing → planning → in_progress → review → done`) and an idempotent branch edge (`git switch -c … \|\| git switch …`); migrate the forward backlog; task-aware gates | mtt tracks its own tasks; `done` gated on `make check` |
