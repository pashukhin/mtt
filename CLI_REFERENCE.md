...
- `--template <name|path|url>` *(t62)* — the starter config, resolved by shape:
  ...
  - **Untrusted config-as-code:** an external template's gate/post commands run via `sh -c` on your
    ...
    `templates/` dir (`coding`, `hierarchy`, the `git-flow` flagship, and the `github-gated` flagship —
    puts executable mtt gates on a GitHub issue's close; needs `gh`+`jq`) — install by path or URL.
