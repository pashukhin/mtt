...
- **Инварианты (валидирует загрузка конфига):** в наборе типов есть дефолтный `task`; в любом
  flow есть `tbd → in_progress → done` именно в этом порядке (промежуточные допустимы).
