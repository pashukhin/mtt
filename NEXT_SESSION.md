...
- **Capabilities:** возможности (история, зависимости, дерево-комментарии, поиск) — опциональны
  per-адаптер (`Capabilities()` / `ErrUnsupported`); YAML — референс (умеет всё), `core` пишет на
  минимум и «зажигает» доступное. Задача несёт append-only `history` (в YAML пишем всегда).
- **Killer-фича — исполняемые переходы:** на переход вешаются `description` + `commands`
  ...
  `internal/adapter/exec` реализует, тест — фейк). Фаза 3.
