...
| 1 | **Чистый** контракт `pkg/mtt` (домен-типы + порт `TaskStore`); конфиг+типы (**структурные** инварианты: `kind` по топологии, ≥1 каждой, без литералов имён), `mtt init`; YAML-адаптер **чеканит плоские per-prefix ID** (`e1`/`t17`/`s3`); core-usecase + `add/list/show/edit/close` | |
| 2 | Иерархия (по `parents` из конфига); зависимости; `ready`; детект циклов | |
