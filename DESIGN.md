...
per-command timeout (which SIGKILLs the command's **whole process group** on Unix — so a gate that backgrounds
a daemon can't outlive its deadline; best-effort on Windows); the escape hatch `--no-run` forces the
transition without commands (for emergencies).
