...
- `statuses` / `transitions` — flow (см. ниже); входной статус — канонический `tbd`.
...
- В любом flow есть канонические статусы **`tbd → in_progress → done`** именно в этом порядке
  (промежуточные допустимы). `tbd` — вход, `done` — терминал.
- В коде **нет** литералов имён типов и структуры ID: типы/иерархия — из конфига (домен),
  кодировка ID — из адаптера. Дефолты живут в шаблоне `mtt init`, а не в логике.
...
`mtt init` пишет `.mtt/config.yaml` с типами `epic`/`task`/`subtask` и flow `tbd → in_progress
→ done`. **Команд по умолчанию нет** — их навешивает пользователь под свой проект.
