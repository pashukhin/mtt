...
- `Match` (pure predicate): reports whether a task satisfies a `ListFilter` — status/type/kind/**priority**/parent (AND
...
- `Roadmap` (pure read, s008.6): `Roadmap(tasks, cfg) []RoadmapEntry` — the non-terminal tasks in a
  dependency-respecting, priority-weighted order (a **priority-guided Kahn** topo sort): a `depends_on` edge is
  a **hard** constraint (a non-terminal blocker precedes its dependent, even at lower priority), priority is a
  **soft** tiebreak (`lessByPriority`, so roadmap == a dependency-constrained `list --sort priority`). Each
  `RoadmapEntry` carries `Ready` (core.Ready membership — single source of truth) + `BlockedBy` (depends_on not
  `terminalSatisfied`). No store/clock, **not** in the `pkg/mtt` contract. It builds its **own**
  non-terminal-restricted DAG (**not** `DepGraph`, whose `Dependents` are unfiltered — GAP #6 stays
  unextracted). **Cycle-safe**: a node in — or downstream of — a `depends_on` cycle is appended best-effort so
  the function always terminates and returns every node.
...
- `Transitioner` (the flow-gate usecase, a mutation):
  ...
  walk), then (s006.5) enforces **required-attribution** — `missingAttribution(opts)` aggregates the absent
  `who`/`why` into one `ErrMissingAttribution` **before the gate** (fail fast; `NoRun` does **not** bypass it) —
  ...
  **succeeded-prefix** rollbacks **in reverse** via `Runner.Compensate` (`rollbacksBefore`); the failed command's
  ...
  (exit 3), task unchanged, no history; the block error carries a `compSummary` (`compensated N …`). `ErrBlocked`/`ErrInvalidTransition`/`ErrMissingAttribution` are core
  sentinels (flow/attribution is core policy); the CLI maps them to exit codes 3/6/2. Since s006.7,
