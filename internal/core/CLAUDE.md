...
- `Match` (pure predicate): reports whether a task satisfies a `ListFilter` — status/type/kind/**priority**/**tags**/parent (AND
  across dimensions, OR within). The **Tags** dimension (s008.7) is slice-valued — a task matches if it carries
  **any** filter tag (`anyOrEmptyIntersect`, the slice analogue of scalar `anyOrEmpty`). `cfg` is consulted only for the kind dimension (resolve the task's status
...
- `TagEditor` (mutation, s008.7): `AddTags`/`RemoveTags` edit `Task.Tags` and persist via `TaskStore.Update`
  (**no new port** — GAP #1, like `depends_on`). Both idempotent; `RemoveTags` is **guarded** — it refuses a
...
- `Remover` (the `rm` usecase, a mutation): `RemoveMany(ids, force) []RemoveResult` is the primary method —
  ...
  in-set child/dependent does not block). `Remove(id, force)` is a thin **wrapper** over `RemoveMany([id])`
  (set={id} ⇒ every referent external ⇒ identical single-id reject/exit-4 semantics). No clock (a delete
  records nothing). `--force` leaves dangling refs, which the system tolerates (`Ready` conservative; `Index`
