...
- `Transitioner` (the flow-gate usecase, a mutation):
  ...
  **succeeded-prefix** rollbacks **in reverse** via `Runner.Compensate` (`rollbacksBefore`); the failed command's
  ...
  post failure returns the **persisted** task with `ErrPostAction` (the **single** case a non-nil error carries a
  valid task; move kept, no rollback, no history for post checks); the CLI keeps the move and maps it to exit 5.
