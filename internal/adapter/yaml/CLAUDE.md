...
- `Init` — render the sole **embedded** built-in `default` (sourced from the repo-root `templates` package,
  `text/template` `{{.Name}}`; an unknown built-in name errors with the sorted valid names via `templateNames()` — c14),
  then `writeConfig`. **(t62)** `coding`/`hierarchy` are **de-embedded** — no longer built-in NAMES; only `default`
  renders here. `renderTemplate` (`templates.go`) sources `default` from `roottemplates.Default()`.
  Templating/name-resolution: **only `Init`'s built-in path** runs `text/template`; external templates never
  reach it (verbatim). `writeConfig` (extracted, shared) does the **atomic** write (temp+rename), the
  refuse-overwrite-without-force check, and drops `.mtt/.gitignore` ignoring the personal `config.local.yaml`
  overlay (c11) — create-if-absent via `O_EXCL`: an existing file is the user's, never clobbered (even under
  force); the project root `.gitignore` is untouched. Written **before** the config existence check, so a
  re-init (even one failing `ErrAlreadyInitialized`) heals a missing `.gitignore`; a failed write removes the
  partial file (the `ErrExist` skip must never see a corpse).
- **`--template` sources (t62):** `ClassifyTemplate(arg)` (`templatesource.go`, **pure**) resolves the arg by
  shape → `SourceBuiltin` (bare name) / `SourceFile` (separator or `.yaml`/`.yml` suffix) / `SourceURL`
  (`https://`; `http://` errors). `InstallConfig(root, data, force)` is the **external** path: it
  `ValidateTemplateBytes` (fail-closed) then `writeConfig` **verbatim** (no `{{.Name}}` render — external
  configs carry `{{.ID}}`/`{{.Type}}` runtime placeholders). `Fetcher` (`templatefetch.go`) downloads a URL
  behind an injectable `http.RoundTripper` (a real `http.Client` so `httpsOnlyRedirect` runs on redirects) —
  **https-only**, ~30s timeout, ~1 MiB cap (`LimitReader(cap+1)` so over-cap errors, not silently truncates),
  non-200 → error. The confirm-prompt/TTY logic is a CLI concern.
- `Load` — read config + optional gitignored `config.local.yaml` overlay (later wins at top-level-field granularity: a scalar like `project.name` overrides, but a list such as `types` replaces wholesale — no element-level merge), map DTO→domain, run provider checks. **(t62)** the post-overlay checks are factored into **`checkDecoded(yc)`** (`toDomain` + `checkPrefixes` — exactly one `default`; prefix present+unique+**letters-only** `prefixPattern`, the mint-side half of the id charset so every minted `<prefix><N>` satisfies `idPattern` — + `parseCommandTimeout`); `Load` routes through it and stays **byte-identical** (overlay, `committedRequire`, `Author`, tighten-only `require` all unchanged). Domain `Config.Validate()` is **NOT** run by `Load` (it is the caller's — only `add`/`types` validate); the external-template validator **`ValidateTemplateBytes(data)`** = single-doc decode → `checkDecoded` → `Config.Validate` (init legitimately owns the domain check for an untrusted template). Returns `Settings{Prefixes, CommandTimeout, Author, Require, ExtractHashtags}`: `ExtractHashtags` (`extract_hashtags`, t69; default **off**, read post-overlay like `author`) is the committed `#hashtag`→tag extraction policy the CLI threads into `core`. `Require` (`require:{who,why}`, s006.5) is the required-attribution policy — **tighten-only**, so the committed `require` is captured **before** the local overlay and OR-combined (`committed || local`): `config.local` can add a requirement but never relax a committed one.
- `NewTaskStore(root)` / `Store` — implements `mtt.TaskStore`. `Create` mints a **flat per-prefix** ID (`<prefix><N>` via `mint`, scan `max+1`, `O_EXCL` reserve), serializes the `ymlTask` DTO (RFC3339 UTC, `omitempty` on reserved fields), and writes atomically to `.mtt/tasks/<id>.yaml`. `Get` reads/maps a task, returning `mtt.ErrNotFound` when absent. IDs are flat (no parent chain) → stable under re-parenting; identity lives in the ID, hierarchy in the `parent` field. `List` reads `.mtt/tasks/*.yaml` → domain (order unspecified; `core` orders). Both read paths share `parseTaskFile(path, data)` (unmarshal → **id guard** → `toDomain`), which enforces the store's **load invariant** before mapping (**c15**): the in-file `id:` must equal the **filename stem** (fail-closed on the duplicate-id split-brain — a copied file that kept the other's inner id) **and** match the adapter's id encoding `^[a-zA-Z]+[0-9]+$` (`idPattern` — `<prefix><N>`). The charset guard is the **shell-safety** boundary: a loaded id is expanded into `{{.ID}}` inside gate/post `sh -c` commands, so an id carrying shell metacharacters/whitespace is refused at load, before any command runs — the domain `NewTaskID` stays opaque/permissive (this "prefix+digits" charset is **adapter** encoding, not a domain rule). A `toDomain` failure (a corrupt task file) is wrapped with the **offending file path** in both `List` and `Get` (`fmt.Errorf("%s: %w", path, err)`, A1) so it is a one-command fix at volume; the `ErrNotFound` (absent-file) path is untouched, so a genuinely missing task still maps to exit 4. A **zero-byte** file is NOT corruption but the **mint reserve-window artifact** (a crash between the `O_EXCL` reserve and the content write, c18): every port path treats it as absent — `List` skips it, `Get`/`Update`/`Delete` map it to `ErrNotFound` (an `Update` must not resurrect what `Get` can't see; a `Delete` must not free the consumed id back to mint) — and mint's filename scan still counts it, so the reserved id stays consumed forever (a reuse would silently re-point dangling references). `Update` overwrites an existing task by ID (`ErrNotFound` if absent); its existence check (`os.Stat` then write) is a check-then-act with a theoretical TOCTOU window, acceptable for the single-user local CLI (same filesystem assumptions as `Create`). `Create`/`Update` share one private `write` (marshal + atomic temp+rename) — serialization lives in exactly one place. **Durability (c18):** the shared `atomicWrite` (= `atomicWriteMode(path, data, filePerm)`) follows the installer's discipline — write → `Chmod` → `Sync` → close → rename → **parent-dir fsync** (best-effort where a dir handle can't sync: Windows, unsupported, EINVAL mounts). **(t52)** this discipline now lives in `internal/fsutil` (`AtomicWrite`/`SyncDir`); the adapter's `atomicWrite`/`atomicWriteMode` are thin delegations and `audit.go` calls `fsutil.SyncDir` directly — one durability implementation, same behavior. Perm policy: an **existing target's mode is preserved** (installer-style — the store never loosens a user-tightened file); a NEW file lands `filePerm` `0o644` (the git-checkout default, so fresh writes and checkouts agree cross-machine), except a fresh `config.local.yaml`, which `Current` writes via `atomicWriteMode(…, 0o600)` — the personal overlay is the documented home for data up to backend credentials. `AuditStore.Append` fsyncs the line **and** the `.mtt` dirent before returning — the record is the precondition for a destructive delete, including the first-ever append that creates the file. `Delete` removes `.mtt/tasks/<id>.yaml` via `os.Remove` (`ErrNotFound` when absent); the unlink is atomic, same filesystem assumptions as the writes.
- `NewKnowledgeStore(root)` / `NoteStore` (t47) — implements `mtt.KnowledgeStore` over `.mtt/knowledge/<slug>.md`, one markdown file per note. **Serialization contract** (`note_dto.go`, `marshalNote`/`parseNote`): `"---\n"` + a **struct** frontmatter DTO (`ymlNote{title,tags,created,updated}` — deterministic order, `omitempty` title/tags; slug is the **file name**, never in frontmatter) + `"---\n"` + the body **verbatim** (trailing-newline preserved). Read splits on the **first** closing `---` and unmarshals **only** the frontmatter — **never** the whole file (`\n---\n` is yaml's doc separator), so `---` inside the body is safe; a file not starting with `---` is a load error (not `ErrNotFound`). `CreateNote` is **reserve-then-write** (`O_CREATE|O_EXCL` on the final path — mirroring `mint` — then `atomicWrite`), so an existing slug errors without clobbering; a **zero-byte** `.md` is its reserve-window artifact (c18) — `ListNotes` skips it, `GetNote` maps it to `ErrNotFound` (same policy as the task store). Every path-building method (`Create`/`Get`/`Update`/`Delete`) and `ListNotes` **re-validate** the slug (`NoteSlug.Valid()` / `NewNoteSlug` on the filename) — defense-in-depth against a raw `NoteSlug("../x")` cast. Golden `note_min.md`/`note_full.md` pin the layout. No versioning/search (t6). **(t1)** `ymlNote.Refs`
