...
gate commands and BLOCKS on failure, recording history. Per-type flows + gates
live in the committed .mtt/config.yaml, so the "definition of done" is repo data.
...
status + next moves. All commands support --json.`,
	...
	root.AddCommand(newVersionCmd(), newInitCmd(), newTypesCmd(), newAddCmd(), newShowCmd(),
		...
		newNoteCmd(), newRefCmd(), newCheckCmd(), newPrimeCmd())
	return root
}
