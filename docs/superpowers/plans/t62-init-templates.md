...
- [ ] **Step 7: Migrate the demo.** In `demo/coding-flow.sh`, **self-locate** the repo root (so it works standalone, via `make demo`, AND via the Go test — the script runs under `set -eu`, so an env-only `REPO_ROOT` would be an unbound-variable hard error for `make demo`, which does not pass it): add near the top `REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"`. Then before the init, materialize the example: `cp "$REPO_ROOT/templates/coding.yaml" ./coding.yaml` and `"$MTT" init --template ./coding.yaml`. (The Go test may still set `REPO_ROOT` in `cmd.Env`, but the self-locate default makes it optional. `init_test.go:31` was already migrated in Step 4.)
