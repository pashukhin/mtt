...
2. **Working under mtt** — cut to pointers (the flow prints these): the two-type litmus sentence (keep
   ...
   note"). KEEP: backlog navigation, attribution setup, mid-flight resumption, dangerous-ops summary,
   auto-commit/auto-push + exit-5 recovery, config-is-code.
...
4. **Git section**: `Branches:` bullet → flow-created `task/<id>` for all mtt work; `feat/…`, `fix/…`,
   `chore/…` remain for non-task exceptions (bootstrap/infra) — all three kept, deliberately (review note).
...
Move into touched statuses / `mtt show` prints the D1 texts; guard green incl. 4 spot-checks; AGENTS.md
